...
//-------------------------------------------------------------------------------------------------
    ...
    public void remove(E element) {
        root = removeRec(element, (AVLNode) root);
    }

    private AVLNode<E> removeRec(E element, AVLNode<E> node) {
        if (node == null) {
            return node;
        }
        ...
        if (cmp < 0) {
            node.left = removeRec(element, node.getLeft());
        } else if (cmp > 0) {
            node.right = removeRec(element, node.getRight());
        } 
        else { 
            if (node.left == null) {
            ...
            node.right = removeRec(node.element, node.getRight());
        }
