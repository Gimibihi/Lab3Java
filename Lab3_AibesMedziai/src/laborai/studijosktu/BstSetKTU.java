...
//---------------------------------------------------------------------------------------
    ...
    /**
     * Grąžinamas aibės poaibis po elemento.
     *
     * @param element - Aibės elementas.
    ...
    public SetADT<E> tailSet(E element) {
        ...
        BstNode<E> tailRoot = getNode(element);
        BstSetKTU<E> tail = this;
        tail.setRoot(tailRoot);
        return tail;
    }
    /*
    Returns node of an element
    */
    BstNode<E> getNode(E element){
        if (element == null) {
            throw new IllegalArgumentException("Element is null in get(E element)");
        }
...
//------------------------------------------------------------------------------------------
    ...
    int getLeafCount(BstNode<E> node){
        if (node == null) 
            return 0; 
