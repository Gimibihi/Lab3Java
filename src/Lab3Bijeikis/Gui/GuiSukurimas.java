package Lab3Bijeikis.Gui;
import Lab3Bijeikis.DuomGeneravimas;
import Lab3Bijeikis.GreitaveikosTyrimas;
import Lab3Bijeikis.Kompiuteris;
import Lab3Bijeikis.Testai;
 ...
 */
public class GuiSukurimas extends JFrame implements ActionListener {
    ...
    private SortedSetADTx<Kompiuteris> kompSet;
    ...
    private void appearance() {
        ...
        panParam1.getTfOfTable().get(4).setBackground(Color.GRAY);
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (source.equals(panButtons.getButtons().get(0))) {
                ...
                KsSwing.oun(taOutput, String.valueOf(kompSet.findAmountOfLeafNodes()));
            }
    ...
    public void headSet() throws Exception {
        if (kompSet != null) {
            try {
                ...
                Kompiuteris element = DuomGeneravimas.gautiIsBazes();

                BstSetKTU<Kompiuteris> headSet = (BstSetKTU<Kompiuteris>) kompSet.headSet(element);

                KsSwing.setFormatStartOfLine(true);
                KsSwing.oun(taOutput, MESSAGES.getString("msg9"));
                KsSwing.oun(taOutput, headSet.toVisualizedString(delimiter));
    ...
    public void treeGeneration(String filePath) throws Exception {
        ...
        Kompiuteris[] kompArray;
        // Jei failas nenurodytas - generuojama
        if (filePath == null) {
            kompArray = DuomGeneravimas.generuotiIrIsmaisyti(sizeOfGenSet,sizeOfInitialSubSet,coef);
            ...
            kompSet.load(filePath);
            kompArray = new Kompiuteris[kompSet.size()];
            int i = 0;
            for (Object o : kompSet.toArray()) {
                kompArray[i++] = (Kompiuteris) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(kompArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        kompSet.clear();
        for (Kompiuteris komp : kompArray) {
            kompSet.add(komp);
        }
        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, kompSet.toVisualizedString(delimiter),
    ...
    private void treeAdd() throws Exception {
        Kompiuteris komp = DuomGeneravimas.gautiIsBazes();
        kompSet.add(komp);
        panParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, komp, MESSAGES.getString("msg7"));
        KsSwing.oun(taOutput, kompSet.toVisualizedString(delimiter));
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (kompSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(kompSet.size());
            Kompiuteris komp = (Kompiuteris) kompSet.toArray()[nr];
            kompSet.remove(komp);
            KsSwing.oun(taOutput, komp, MESSAGES.getString("msg6"));
            KsSwing.oun(taOutput, kompSet.toVisualizedString(delimiter));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (kompSet.isEmpty()) {
            ...
            KsSwing.oun(taOutput, kompSet, MESSAGES.getString("msg8"));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectedIndex()) {
            case 0:
                kompSet = new BstSetKTUx(new Kompiuteris());
                break;
            case 1:
                kompSet = new AvlSetKTUx(new Kompiuteris());
                break;
            default:
                enableButtons(false);
    ...
    public static void createAndShowGUI() {
        ...
        GuiSukurimas window = new GuiSukurimas();
