...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public int findAmountOfLeafNodes(){
        return findAmountoFLeafNodes(root);
    }
    private int findAmountoFLeafNodes(BstNode<E> node) {
        if (node == null) {
            return 0;
        }
        ...
        int indexL = findAmountoFLeafNodes(node.left);
        int indexR = findAmountoFLeafNodes(node.right);
    ...
    public SetADT<E> headSet(E element) {
        ...
        BstSetKTU<E> result = new BstSetKTU<E>(c);
        addToHeadSet(root, element, result);
        return result;
    }
    private void addToHeadSet(BstNode<E> node, E element, BstSetKTU<E> result) {
        if (node == null) {
            return;
        }
        ...
        if (cmp > 0) {
            // The current node's element is greater than the given element,
            // so we add it to the result set.
            result.add(node.element);
            addToHeadSet(node.right, element, result); // Recurse into the right subtree
        } else if (cmp < 0) {
            // The current node's element is less than the given element,
            // so we only recurse into the left subtree.
            addToHeadSet(node.left, element, result);
        } else {
            // The current node's element is equal to the given element.
            // We add the left subtree (elements less than the given element) to the result.
            result.root = node.left;
        }
    }
